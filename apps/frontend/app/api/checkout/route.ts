...
export async function POST(req: Request) {
  ...
  if (!resp.ok || !json.url) {
    return Response.json({ error: json?.error?.message ?? "Stripe error" }, { status: 500 });
  }
